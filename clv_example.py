...
def CLV(universe, start, end, file_name):
    ...
    for stk in universe:
        ...
        tmp_factor_data['CLV'] = ((tmp_factor_data['close'] - tmp_factor_data['low']) - (
            tmp_factor_data['high'] - tmp_factor_data['close'])) / (tmp_factor_data['high'] - tmp_factor_data['low'])
        # 处理出现一字涨跌停
...
def getMarketValueAll(universe, start, end, file_name):
    ...
    logger.debug("获得从 %s~%s %d只股票的市值信息%d条",start,end,len(universe),len(ret_data))
...
def getForwardReturns(universe, start, end, window, file_name):
    ...
    ret_data.to_csv(file_name)
    logger.debug("保存5天因子收益率到文件：", file_name)
    return ret_data
...
logger.debug("获得 %d 只股票 %s~%s 的5日收益率信息",len(universe),start_date,end_date)
...
logger.debug("IC mean：%.4f" % ic_data['IC'].mean())
logger.debug("IC median：%.4f" % ic_data['IC'].median())
logger.debug("IC %d>0, %d<0" % (len(ic_data[ic_data.IC > 0]), len(ic_data[ic_data.IC < 0])))
