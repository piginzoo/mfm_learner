...
def momentum(period, df):
    ...
    df['momentum'] = np.log(adj_close / adj_close.shift(period))  # shift(1) 往后移，就变成上个月的了
    return df
...
def main(stock_pool, start, end, adjustment_days, stock_num):
    """
    ...
    """
    stock_codes = get_stock_names(stock_pool, start, stock_num)
    assert stock_codes is not None and len(stock_codes) > 0, stock_codes
    data = load_stock_data(stock_codes, start, end)
    logger.debug("获得%s~%s %d 条因子",start,end,len(factors))
    ...
    factor_data = get_clean_factor_and_forward_returns(factors, close, periods=[])
...
if __name__ == '__main__':
    ...
    stock_num = 3  # 用股票池中的几只，初期调试设置小10，后期可以调成全部
