...
def test_factor(factor, stock_index, begin_date_list, end_date_list, layer_sign=0):
    """
    :param factor:  待测的单因子
    :param stock_index : 股市指数，比如沪深300（hs300）
    ...
    for i in range(len(begin_date_list)):
        ...
        # code_list = at.get_code_list(stock_index, date=begin_date_list[i])
        import tushare
        code_list = tushare.pro_api().index_weight(index_code=stock_index,
                                                   start_date=begin_date_list[i],
                                                   end_date=end_date_list[i])

        code_list = stock_layered(code_list, layer_sign)  # 分层

        # 若要分层回测，这里需要股票池划分
        target_list = code_list['con_code'].tolist()  # 本月股票池代码
        weight_list = np.array(code_list['weight'].tolist())  # 本月各股票权重
        # 获取因子月初数据
        print("{} - {}: 获取因子数据！".format(begin_date_list[i], end_date_list[i]))
        factor_data = get_factor_by_day(factor_list=[factor], target_list=target_list, date=begin_date_list[i])
        ...
        factor_data[factor] = preprocessing.scale(factor_data[factor])  # z-score 规范化
...
def main():
    result = test_all_factors(["CLV"], 'hs300', '2019-01', '2021-01', layer_sign=0)  # 0 不分层  1 低流通市值  2 中流通市值  3 高流通市值
